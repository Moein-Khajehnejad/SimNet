...
#Note
a.In case the adjacency matrix representing the network has eigenvalues larger than one, simply divide all elements by N.max(maxA)). It will guarantee sum of each row will be lower than one, hence there will be no eigenvalue larger than one. Therefore the condition alpha<1 will be sufficient  for the similarity limit to converge.  
